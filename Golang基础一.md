...
- 整数部分，直接转为二进制。
- 小数部分，让小数部分乘以2，结果小于1，则将结果继续乘以2。结果大于1则结果-1继续乘以2，直到结果等于1则结束。

小数部分二进制表示将相乘之后等结果的整数部分拼接起来
...
​	（5）声明指针时要指明指针指向内存区域的类型，因为不同类型，通过指针去取值的时候取多大的连续内存空间是不同的。
...
​		引用类型：指针、切片、map、管道channel、interface、函数
...
​		^ 按位异或：两位一个为0一个为1，则为1，否则为0；可以理解为无进位相加 
​							 任何一个数异或本身为0，任何一个数据异或0为本身
