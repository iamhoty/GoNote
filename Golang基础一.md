...
用来给函数运行提供内存空间，取内存与stack上。

当函数调用时，产生栈帧。函数结束时，栈帧空间释放。

栈帧存储：1.局部变量。 2.形参（形参与局部变量存储地位等同）。 3.内存字段描述值（栈基指针，栈顶指针）。

默认stack栈帧空间大小是1M-8M

heap堆空间1G+，认为编程的时候是无限，**使用完需要立即释放**，Go语言的垃圾回收机制会帮我们回收
...
传(地址)引用：将形参的地址值作用函数参数、返回值后传递。**在A栈帧内部，修改B栈帧中变量的值。**
